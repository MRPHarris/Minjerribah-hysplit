...
# Calculate bearings from cluster termination points to specified point.
clus_bearings <- function(cluster_results,
                          target_point){
  # vars for intra-function testing
  # cluster_results = sc_4clus
  # target_point = c(SC_lon,SC_lat)
  #
...
## Clustering for Sandy cape and point lookout.
...
Frequencies were coerced to the annual scale to provide consistent 
...
Fig 1. Trajectory clusters (n = 5) to Sandy Cape (left) and Point Lookout (right). Frequencies display the >100 mm within each cluster as a proportion of the total number at each site. Cluster names have no statistical significance and were designated to draw parity between each site where possible. Clusters are named based upon their approximate direction of travel. Note that this naming scheme (and any cluster designation method) is inherently and unavoidably reductive, and each cluster will be comprised of trajectories that, whilst similar spatially, will travel in different ways and from different directions. The clustering simply serves to reduce the dimensionality of the data in a meaningful way.
...
Cluster event time series (N/A for now)
...
Proportion (frequencies) overall
...
rainfall_hist <- ggplot() + 
  geom_bar(data = rainfall_all_event_sums %>% filter(!is.na(ENSO_phase)), 
...
rainfall_norm_hist  <- ggplot() + 
  geom_bar(data = rainfall_all_event_sums_norm %>% filter(!is.na(ENSO_phase)), 
                 aes(x = site, y = mean_rainfall_perevent, group = ENSO_phase, fill = ENSO_phase), stat = 'identity', position = 'dodge') +
...
largerainfall_hist  <- ggplot() + 
  geom_bar(data = rainfall_100mm_event_sums %>% filter(!is.na(ENSO_phase)), 
                 aes(x = site, y = mean_rainfall_perevent, group = ENSO_phase, fill = ENSO_phase), stat = 'identity', position = 'dodge') +
...
largerainfall_norm_hist  <- ggplot() + 
  geom_bar(data = rainfall_100mm_event_sums_norm %>% filter(!is.na(ENSO_phase)), 
                 aes(x = site, y = mean_rainfall_perevent, group = ENSO_phase, fill = ENSO_phase), stat = 'identity', position = 'dodge') +
...
Proportion (frequencies) during different ENSO phases

```{r cluster-freq-proportions, echo=FALSE, message=FALSE, results='hide', warning = FALSE, fig.height = 6, fig.width = 14, units = cm}
## Section notes
# Ok, so how do these vary in different phases?
...
Rainfall correlations in different ENSO phases (with seasonal break down)

Plotting specific trajectories



```{r trajectory-path-plots, echo=FALSE, message=FALSE, results='hide', warning = FALSE, fig.height = 7, fig.width = 7, units = cm}
## Section notes
# Plotting the cluster pathways for each site.
...
PL_event_20220329 <- PL_72hr1TPD2000m_Reanalysis %>%
  # ungroup() %>%
  filter(date.start %in% c("2022-03-26","2022-03-27","2022-03-28","2022-03-29","2022-03-30","2022-03-31","2022-04-01"))

# clus_sc_dat <- sc_4clus$data$results
event_plot <- ggplot() +
  EastAus_Full_Basemap +
  geom_path(data = PL_event_20220329, aes(x = lon, y = lat, group = trajectory, colour = trajectory), size = 1, alpha = 1) +
  geom_map(data = EastAus_Zoomout_Data, map = EastAus_Zoomout_Data,
           aes(x = long, y = lat, group = group, map_id = region),
           fill = "transparent", colour = "grey8", size = 0.2) +
  # labs(title = "Sandy Cape >100mm rainfall event clusters (n = 5)") +
