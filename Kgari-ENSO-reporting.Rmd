...
# Make vector of dates for 100mm PL rainfall days.
PL_100mm_dates <- PLCombined_rainfall_data$date_yyyymmdd[which(PLCombined_rainfall_data$Rainfall.amount..millimetres. > 100)]
DW_100mm_dates <- DW_rainfall_data$date_yyyymmdd[which(DW_rainfall_data$Rainfall.amount..millimetres. > 100)] %>% ymd()
# Some are shared!
# Need to know if there are any days at DW which align with multi-day PL 100mm collections. Can these give a 'true' date, and validate the exclusion of certain days?

## Narrow to PL 100mm events that have a measurement period of >1 day. Then, generate a sequence
# of dates that encompasses all days within the measurement windows. Find dates within these that match DW events.
PL_100mm_g1d <- PLCombined_rainfall_data %>%
  filter(Rainfall.amount..millimetres. > 100) %>%
  filter(Period.over.which.rainfall.was.measured..days. > 1) %>%
  mutate(date_yyyymmdd = lubridate::ymd(date_yyyymmdd)) 

## Generate sequence of possible dates containing rainfall from multi-day events at PL
PL_100mm_g1d_days <- date_seq(end_date = PL_100mm_g1d$date_yyyymmdd,
                              back_duration = PL_100mm_g1d$Period.over.which.rainfall.was.measured..days.,
                              include_end = TRUE)

# Matches?
PL_100mm_g1d_days[which(DW_100mm_dates %in% PL_100mm_g1d_days)]
# Get DW days that fall inside these dates.
DW_100mm_PLg1dmatches <- DW_100mm[which(DW_100mm_dates %in% PL_100mm_g1d_days),] 
# Ok, there are four days for certain where there were large single-day events at DW.


# Sequential sums if >100mm 
# What if a rain event falls either side of a collection time? Two-day lag/lead sums
PLCombined_rainfall_data <- PLCombined_rainfall_data %>%
  ...
  mutate(seq_rain_lead = Rainfall.amount..millimetres. + lead(Rainfall.amount..millimetres., n = 1, default = 0))
## SC
## First, account for multi-day events.
SC_rainfall_data <- SC_rainfall_data %>%
  mutate(Period.over.which.rainfall.was.measured..days. = ifelse(is.na(Period.over.which.rainfall.was.measured..days.),
                                                                yes = 0,
                                                                no = Period.over.which.rainfall.was.measured..days.)) %>%
  mutate(Rainfall.amount..millimetres. = ifelse(Period.over.which.rainfall.was.measured..days. > 1,
                                                yes = Rainfall.amount..millimetres./(Period.over.which.rainfall.was.measured..days./2),
...
SC_rainfall_data <- SC_rainfall_data %>%
  ...
  mutate(lag_100_adj = ifelse(Rainfall.amount..millimetres. > 100,
                              yes = 1,
                              no = lag_100_adj)) %>%
    dplyr::select(-c(lag_100_dup,lag_100,seq_rain_lag,rem_days_rfall))
    
## Need to find the largest rain day in each pair.
# Could just say that if the day's rain is less than half of the lagged sum total, send the index back 1? 
SC_rainfall_data <- SC_rainfall_data %>%
...
# Great, now isolate 100mm events! 
SC_rainfall_100mm <- SC_rainfall_data %>%
  filter(lag_100_adj == 1) 

# Ok, need to set these to 0, and the row before them to 1.

## Great, got the dates accounting for heavy rainfall across two measurements.
# Now need to factor in multi-day collections! These are tricky because there is no way to know where the rain fell in the dates it covers.
# SC to test.


SC_rainfall_100mm_old <- SC_rainfall_data %>%
  filter(Rainfall.amount..millimetres. > 100)

SC_rainfall_data$date_yyyymmdd[which(!SC_rainfall_100mm_old$date_yyyymmdd %in% SC_rainfall_100mm$date_yyyymmdd)]

which(SC_rainfall_100mm$Period.over.which.rainfall.was.measured..days. > 1)

SC_rainfall_data$date_yyyymmdd[which(SC_rainfall_data$Period.over.which.rainfall.was.measured..days. > 1)]
    
## NOW NEED TO REMOVE DUPLICATES - i.e., cases where there are two subsequent registers.
# This is fairly common.

## SC pass with double duplicated call
# Or I could just cumsum and find where the values equal 2? 

SC_rainfall_data <- SC_rainfall_data %>%
  mutate(lag_100_dup = lag_100 + lead(lag_100)) %>%
  mutate(lag_100 = ifelse(lag_100_dup == 2,
                          yes = 0,
                          no = lag_100))
...
ggplot() + 
  geom_point(data = PLCombined_rainfall_data, aes(x = date_yyyymmdd, y = Rainfall.amount..millimetres.))

# Compute days with >100mm in either lag or lead
PL_laglead_100mm_days <- length(PLCombined_rainfall_data$date_yyyymmdd[which(PLCombined_rainfall_data$seq_rain_lag > 100 | PLCombined_rainfall_data$seq_rain_lead > 100)])

SC_laglead_100mm_days <- length(SC_rainfall_data$date_yyyymmdd[which(SC_rainfall_data$seq_rain_lag > 100 | SC_rainfall_data$seq_rain_lead > 100)])

## Isolate days where there is 100mm on a given day, or across two days (lagged).
SC_rainfall_data <- SC_rainfall_data %>%
  mutate(lag_100 = ifelse(test = Rainfall.amount..millimetres. > 100 | seq_rain_lag > 100,
                              yes = 1,
                              no = 0))
PLCombined_rainfall_data <- PLCombined_rainfall_data %>%
  mutate(rem_days_rfall = ifelse(Rainfall.amount..millimetres. > 100,
                                 yes = 0,
                                 no = Rainfall.amount..millimetres.)) %>%
  mutate(lag_100 = ifelse(test = rem_days_rfall > 100 | seq_rain_lag > 100,
                              yes = 1,
                              no = 0))
## NEED TO EXCLUDE DAYS THAT INDIVIDUALLY HAVE >100mm AS THESE WILL BE RE-PICKED UP BY THE LAG CALCULATION ON SUBSEQUENT DAYS.
...
# Collection over multiple days
SC_rainfall_g1d <- SC_rainfall_data[(which(SC_rainfall_data$Period.over.which.rainfall.was.measured..days. > 1)),] %>%
  mutate(rainfall_adj = Rainfall.amount..millimetres./Period.over.which.rainfall.was.measured..days.)
# PL
PL_rainfall_g1d <- PLCombined_rainfall_data[(which(PLCombined_rainfall_data$Period.over.which.rainfall.was.measured..days. > 1)),] %>%
  filter(Rainfall.amount..millimetres. >= 100) %>%
  mutate(rainfall_adj = Rainfall.amount..millimetres./Period.over.which.rainfall.was.measured..days.)

## VALIDATING MULTI-DAY EVENTS AT POINT LOOKOUT WITH DUNWICH DATA
# Get dunwich data
DW_rainfall_data <- read.csv(paste0(proj_dir, "data/BOM rainfall/Dunwich_Data.csv"), header = TRUE) 
# Make vector of dates which are within 1 day of PL 100 mm dates
PL_100mm <- PLCombined_rainfall_data %>%
  filter(Rainfall.amount..millimetres. >= 100) %>%
  mutate(date = ymd_h(paste(Year, Month, Day, sep= ' ')))
PL_100mm_dates <- PL_100mm$
...
event_plot <- ggplot() +
  EastAus_Full_Basemap +
  geom_path(data = PL_event_20220329, aes(x = lon, y = lat, group = as.factor(trajectory), colour = as.factor(trajectory)), size = 0.25, alpha = 1) +
  geom_map(data = EastAus_Zoomout_Data, map = EastAus_Zoomout_Data,
           aes(x = long, y = lat, group = group, map_id = region),
           fill = "transparent", colour = "grey8", size = 0.2) +
