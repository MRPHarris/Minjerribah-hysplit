...
This Rmd file is optimised for figures output as separate tiff files. The figs in this doc may have odd scaling as a result. It renders to ms word for testing purposes. 
...
## Are you exporting figures as TIFFs? If so, they will not render in the word doc.
TIFF = FALSE
...
## Removing outliers from Point lookout dataset
...
# What if we focus >125?
mean(SC_100mm$event_rainfall[which(SC_100mm$event_rainfall > 125)])
mean(PL_100mm$event_rainfall[which(PL_100mm$event_rainfall > 125)])
...
bulk_comp <- rbind(SC_composition,
                   PL_composition) %>%
  mutate(site = factor(site, levels = c("SC","PL"))) #%>%
  # mutate(cluster = factor(cluster, levels = c("C-E","C-NE","C-N","C-W","C-S")))
