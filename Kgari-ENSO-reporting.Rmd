...
This Rmd file/word doc is optimised for figures output as separate tiff files. The figs in this doc may have odd scaling as a result.
...
PLCombined_rainfall_data <- PLCombined_rainfall_data %>%
  mutate(mes_prd_adj = Period.over.which.rainfall.was.measured..days.) %>% # just to shorten the code
...
rainfall_norm_hist  <- ggplot() + 
  geom_bar(data = rainfall_all_event_sums_norm %>% 
             ...
             mutate(site = factor(site, levels = c('SC','PL'))), 
                 aes(x = site, y = mean_rainfall_perevent, group = ENSO_phase, fill = ENSO_phase), stat = 'identity', position = 'dodge') +
  labs(x = "Site", y = "Mean rainfall (mm)", title = paste0("Mean rainfall per ENSO phase","\n","(phase duration normalised)")) +
  rain_bar_themes +
  theme(legend.position = 'none')
...
largerainfall_norm_hist  <- ggplot() + 
  geom_bar(data = rainfall_100mm_event_sums_norm %>% 
             ...
             mutate(site = factor(site, levels = c('SC','PL'))), 
                 aes(x = site, y = mean_rainfall_perevent, group = ENSO_phase, fill = ENSO_phase), stat = 'identity', position = 'dodge') +
  labs(x = "Site", y = "Mean rainfall (mm)", title = paste0("Mean large event rainfall per ENSO phase","\n","(phase duration normalised)")) +
  rain_bar_themes +
  theme(legend.position = 'none')
